var textures = [
        ...
        function init() {
            ...
            var dragControls = new THREE.DragControls(groupObject, camera, renderer.domElement);
            ...
            document.addEventListener('mousedown', IntersectionDrag, false);
        ...
        function onDocumentMouseDown(event) {
            if(!enableMouse){
            console.log("mouse down in sence");
        ...
        function onDocumentMouseUp(event) {
            ...
            if (isDown === false) {
                ...
                if (event.changedTouches) {
                    /*if( isMobile.iOS() ){
                              x = event.touch[0].pageX;
						      y = event.touch[0].pageY;
                        }else{*/
                    x = event.changedTouches[0].pageX;
                    y = event.changedTouches[0].pageY;
                    //                        }
                ...
                mouse2.x = (x / renderer.domElement.clientWidth) * 2 - 1;
                mouse2.y = -(y / renderer.domElement.clientHeight) * 2 + 1;
...
*/
        ...
        function addItemInScence(link,x,y){
            var objectLoader = new THREE.OBJLoader();
            objectLoader.load(link, function(obj) {
                obj.position.set(0,0,0);               
               groupDrag.add(obj);
                console.log(groupDrag);
        })
        }
...
/*Drag on Drop*/
        ...
        function IntersectionDrag(event){
            var x,y;
            if (event.changedTouches) {
            ...
            raycaster.setFromCamera(mouse, camera);
            var intersectsDrag = raycaster.intersectObjects(groupDrag.children,true);
            if(intersectsDrag.length > 0){
               console.log(intersectsDrag[0].object);
            }
